...
// A 10% discount is applied server-side for 2+ adults and 2+ young people in
// any mix (2 teens, 2 children, or 1 of each).
//
// Household rules:
...
const PortalBilling = () => {
  ...
  const manageBilling = async (flow: "manage" | "cancel" = "manage") => {
    const busyKey = flow === "cancel" ? "cancel" : "portal";
    setBusy(busyKey); setCheckoutError(null);
    try {
      const { data, error } = await supabase.functions.invoke("create-billing-portal", { body: { flow } });
      if (error) throw error;
      if (data?.url) { window.location.href = data.url; return; }
  ...
  return (
    <PortalLayout>
      <div className="max-w-2xl mx-auto px-6 pt-12 pb-20">
        ...
        {isMember && (
          <div className="border border-primary/25 bg-primary/5 rounded-sm p-5 mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => void manageBilling("manage")}
                disabled={busy !== null}
                className="text-[10px] font-body font-semibold tracking-widest uppercase border border-foreground/20 hover:border-primary/60 hover:text-primary rounded-sm px-4 py-2.5 shrink-0 flex items-center gap-2 disabled:opacity-60"
              >
                {busy === "portal" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Manage billing"}
              </button>
              <button
                onClick={() => void manageBilling("cancel")}
                disabled={busy !== null}
                className="text-[10px] font-body font-semibold tracking-widest uppercase border border-foreground/20 hover:border-foreground/50 rounded-sm px-4 py-2.5 shrink-0 flex items-center gap-2 disabled:opacity-60"
              >
                {busy === "cancel" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Cancel membership"}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="border border-foreground/10 rounded-sm p-5 mb-6 bg-foreground/[0.02]">
            ...
            {familyDiscount && (
              <p className="text-[11px] font-body text-primary mt-3">
                {FAMILY_DISCOUNT_PERCENT}% family discount applies automatically in Stripe Checkout.
              </p>
            )}
            {!familyDiscount && adults >= 2 && (
              <p className="text-[11px] font-body text-foreground/45 mt-3">
                Add two teen/child places in any mix to receive the {FAMILY_DISCOUNT_PERCENT}% family discount.
              </p>
            )}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {PLANS.map((p) => (
              <div key={p.id} className="border border-foreground/10 rounded-sm p-6 bg-foreground/[0.02] flex flex-col hover:border-primary/40 transition-colors">
                <h2 className="font-display text-2xl tracking-wider text-primary mb-2">{p.name}</h2>
        ...
        {!isMember && (
          <p className="text-[11px] text-foreground/40 font-body mt-4">
            Stripe shows the exact household total before payment. Eligible family discounts are applied automatically. Cancel anytime in two clicks.
          </p>
        )}
