...
const Checkin = () => {
  ...
  useEffect(() => {
    ...
    supabase.from("profiles").select("display_name, name").eq("user_id", user.id).single().then(({ data }) => {
      if (data) setDisplayName((data as any).display_name || data.name || "");
    });
    // Find previous goal
    supabase.from("workbook_entries").select("weekly_goal, session_id").eq("profile_id", user.id).not("weekly_goal", "is", null).order("created_at", { ascending: false }).limit(1).single().then(({ data }) => {
    ...
    <div className="min-h-screen bg-[#0D0B14] flex flex-col items-center justify-center px-5 py-10">
      ...
      <div className="w-full max-w-sm space-y-5">
        <div>
          ...
          <input value={displayName} onChange={(e) => setDisplayName(e.target.value)} placeholder="First name or nickname" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-base font-body placeholder-white/15 focus:outline-none focus:border-white/20" />
        </div>
