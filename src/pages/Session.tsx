...
// Communication Reset session data
  ...
  sections: [
    {
      id: "arrive",
      header: "ARRIVE",
      sub: "Before we begin — a moment to land.",
      duration: "5 MIN",
      activities: [
    ...
    },
    {
      id: "notice",
      header: "NOTICE",
      sub: "The facilitator will guide you through this section.",
      duration: "20 MIN",
      activities: [
    ...
    },
    {
      id: "name",
      header: "NAME",
      sub: "Giving language to what's underneath.",
      duration: "25 MIN",
      activities: [
        {
          type: "connectPairs",
          key: "name-pairs",
          instruction: "Match each emotion to what it might be telling you",
          left: ["Anger", "Sadness", "Fear", "Shame", "Overwhelm"],
          right: ["A boundary was crossed", "A loss occurred", "Something feels threatening", "I believe I'm not enough", "Too much is being asked"],
          correct: [["Anger", "A boundary was crossed"], ["Sadness", "A loss occurred"], ["Fear", "Something feels threatening"], ["Shame", "I believe I'm not enough"], ["Overwhelm", "Too much is being asked"]],
        },
    ...
    {
      id: "sort",
      header: "WHAT CONNECTS AND WHAT CREATES DISTANCE?",
      duration: "15 MIN",
      activities: [
        {
          type: "dragDrop",
          key: "sort-communication",
          instruction: "Sort these communication styles into the correct column",
          cards: ["Blame", "Curiosity", "Listening to respond", "Listening to understand", "Stonewalling", "Naming your feeling", "Criticism", "Saying I need...", "Shutting down", "Staying in your body"],
          columns: [
            { id: "connects", label: "BUILDS CONNECTION", correct: ["Curiosity", "Listening to understand", "Naming your feeling", "Saying I need...", "Staying in your body"] },
            { id: "distance", label: "CREATES DISTANCE", correct: ["Blame", "Listening to respond", "Stonewalling", "Criticism", "Shutting down"] },
          ],
        },
      ],
    },
    {
      id: "rewire",
      header: "REWIRE",
      sub: "One deliberate choice this week.",
      duration: "15 MIN",
      activities: [
    ...
    },
    {
      ...
      duration: "5 MIN",
      activities: [
        { type: "commitment", key: "commitment", stem: "Before next Sunday, I will..." },
      ],
    },
  ],
};
      ...
      <>
        ...
        <div className="section-navy min-h-screen flex items-center justify-center pt-16">
          <div className="text-center">
            <h1 className="heading-display text-5xl text-silver mb-4">SESSION NOT FOUND</h1>
    ...
    session.sections.forEach((section) => {
      ...
      section.activities.forEach((activity) => {
        ...
        if (activity.type === "reflection") {
          notes += `${activity.prompt}\n→ ${val}\n\n`;
        } else if (activity.type === "scale") {
          notes += `${activity.statement}\n→ ${val}/10\n\n`;
        } else if (activity.type === "multipleChoice") {
          notes += `${activity.question}\n→ ${val.selected?.join(", ")}\n${val.reason ? `Why: ${val.reason}\n` : ""}\n`;
        } else if (activity.type === "phraseBuilder") {
          notes += `${activity.template.replace(/________/g, "___")}\n→ ${(val as string[]).join(", ")}\n\n`;
        ...
        <div className="section-navy min-h-screen pt-24 pb-16">
          <div className="container mx-auto px-6 max-w-2xl">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
              ...
              <p className="text-silver/50 text-sm">{session.title} — {session.subtitle}</p>
            </motion.div>

            {/* Summary */}
            <div className="space-y-4 mb-12">
              {session.sections.map((section) => (
                <div key={section.id} className="border-2 border-silver/10 p-6">
                  <h3 className="font-display text-lg tracking-widest text-silver mb-4">{section.header}</h3>
                  {section.activities.map((activity) => {
                    const val = progress[activity.key];
                    if (!val) return null;
                    return (
                      <div key={activity.key} className="mb-3 text-silver/50 text-sm font-body">
                        {activity.type === "reflection" && <><p className="text-silver/30 text-xs mb-1">{activity.prompt}</p><p>"{val}"</p></>}
                        {activity.type === "scale" && <><p className="text-silver/30 text-xs mb-1">{activity.statement}</p><p>{val}/10</p></>}
                        {activity.type === "multipleChoice" && <><p className="text-silver/30 text-xs mb-1">{activity.question}</p><p>{val.selected?.join(", ")}</p></>}
                        {activity.type === "commitment" && val.locked && <p className="text-silver font-display text-lg">"{val.commitment}" — {val.name}</p>}
                      </div>
                    );
                  })}
                </div>
            ...
            <p className="text-center text-silver/30 text-xs tracking-widest mt-12">SEE YOU NEXT SUNDAY</p>
          </div>
        </div>
      </>
    );
    ...
    <>
        ...
        <div className="container mx-auto flex gap-2 overflow-x-auto">
          {session.sections.map((s, i) => (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 text-[10px] tracking-widest whitespace-nowrap border transition-colors ${
                i === currentSection
                  ? "border-silver bg-silver/10 text-silver"
                  : i < currentSection
                  ? "border-silver/20 text-silver/40"
                  : "border-silver/10 text-silver/20"
              }`}
            >
        ...
        <div className="container mx-auto px-4 max-w-2xl">
          ...
          {currentSection === 0 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center mb-12">
              <span className="text-silver/30 text-[10px] tracking-widest">{session.sessionNumber} — {session.duration}</span>
              <h1 className="heading-display text-4xl md:text-6xl mt-2 mb-2">{session.title}</h1>
              <p className="text-silver/40 text-xs tracking-widest">{session.subtitle}</p>
            </motion.div>
          )}
            ...
            >
              <div className="mb-8">
                <div className="flex items-center gap-4 mb-2">
                  ...
                  {sect.duration && <span className="text-silver/20 text-[10px] tracking-widest border border-silver/10 px-2 py-1">{sect.duration}</span>}
                </div>
                {sect.sub && <p className="text-silver/50 text-sm font-body">{sect.sub}</p>}
              </div>

              {/* Activities */}
              <div className="space-y-6">
                {sect.activities.map((activity) => {
              ...
              {/* Navigation */}
              <div className="flex justify-between mt-12 pt-6 border-t border-silver/10">
                <button
                ...
                {isLast ? (
                  ...
                  <button
                    onClick={() => setCurrentSection((p) => Math.min(session.sections.length - 1, p + 1))}
      ...
      {showProgress && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} className="relative w-full max-w-md bg-primary border-l-2 border-silver/20 h-full overflow-y-auto p-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-display text-2xl tracking-widest">MY PROGRESS</h2>
              <button onClick={() => setShowProgress(false)}><X size={24} className="text-silver" /></button>
            </div>
            {session.sections.map((section) => (
              <div key={section.id} className="mb-6">
                <h3 className="font-display text-sm tracking-widest text-silver/40 mb-3">{section.header}</h3>
                {section.activities.map((activity) => {
                  const val = progress[activity.key];
                  if (!val) return <p key={activity.key} className="text-silver/20 text-xs mb-2">—</p>;
                  return (
                    <div key={activity.key} className="text-silver/60 text-xs font-body mb-2 pl-3 border-l border-silver/10">
