      ...
      <div className="max-w-lg mx-auto px-5 pt-6">
        {activeSession && (
          <div className="border border-white/[0.08] p-5 mb-8">
            ...
            <h2 className="font-display text-lg font-bold text-white mb-1">{activeSession.title}</h2>
            {activeEntry?.completed_at ? (
              <div className="flex items-center gap-2 mt-3">
                ...
                <Link to="/workbook" className="ml-auto text-white/20 text-xs font-body hover:text-white/40">Read again →</Link>
              </div>
            ) : (
              <div className="mt-3">
        ...
        {/* 52-week grid */}
        <div className="mb-8">
          <p className="text-white/20 text-[9px] tracking-[0.15em] font-body mb-4">MY YEAR</p>
        ...
        {recentGoals.length > 0 && (
          <div className="mb-8">
            <p className="text-white/20 text-[9px] tracking-[0.15em] font-body mb-4">MY GOALS</p>
            <div className="space-y-2">
              {recentGoals.map((e, i) => {
                const s = sessions.find((s: any) => s.id === e.session_id);
                return (
                  <div key={i} className="flex gap-3 text-sm font-body">
                    <span className="text-white/15 text-xs shrink-0">W{s?.session_number || "?"}</span>
                    <span className="text-white/40">"{e.weekly_goal}"</span>
                  </div>
                );
              })}
            </div>
        ...
        {leavingWords.length > 0 && (
          <div className="mb-8">
            <p className="text-white/20 text-[9px] tracking-[0.15em] font-body mb-4">MY WORD COLLECTION</p>
            <div className="flex flex-wrap gap-2">
              {leavingWords.map((w, i) => (
                <span key={i} className="text-white/30 text-sm font-body border border-white/[0.06] px-3 py-1">{w}</span>
              ))}
            </div>
          </div>
        )}
