...
const AdminSessionEditor = () => {
      ...
      supabase.from("sessions").select("*").eq("id", id).single().then(({ data }) => {
        if (data) {
          setForm({
            ...
            title: data.title || "",
            video_title: (data as any).video_title || "",
            admin_notes: (data as any).admin_notes || "",
            ai_questions: (data as any).ai_questions || null,
            status: data.status,
          });
        }
      });
  ...
  const analyseTeenVideo = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("analyse-video", {
        body: { video_url: kidsTeen.video_url, age_group: "teen" },
      });
      if (error) throw error;
  ...
  const save = async (status?: string) => {
    ...
    const payload: any = {
      ...
      ai_questions: form.ai_questions,
    ...
    if (sessionId && sessionId !== "new") {
      const saveKid = async (ageGroup: string, data: any, existing: any) => {
        ...
        if (existing) {
          await supabase.from("kids_sessions").update(kidPayload).eq("id", existing.id);
        } else if (Object.values(data).some((v: any) => v)) {
          await supabase.from("kids_sessions").insert(kidPayload);
        }
      };
