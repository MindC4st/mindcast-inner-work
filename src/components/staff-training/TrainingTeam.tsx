...
import { Download, Loader2, ShieldCheck, ShieldAlert, Users } from "lucide-react";
...
type ProgressLite = { user_id: string; module_id: string; status: string; score: number | null; expires_at: string | null; due_at: string | null; module_version: number };
...
const TrainingTeam = () => {
  ...
  useEffect(() => {
    (async () => {
      const [{ data: roles }, { data: m }, { data: p }, { data: sr }] = await Promise.all([
        ...
        supabase.from("staff_training_progress" as never).select("user_id, module_id, status, score, expires_at, due_at, module_version").eq("superseded", false),
  ...
  const matrix = useMemo(() => staff.map((s) => {
    ...
    const completed = rows.filter((r) => r.status === "passed" || r.status === "acknowledged").length;
    return {
      staff: s,
      rows,
      completed,
  ...
  const summary = useMemo(() => ({
    total: matrix.length,
    current: matrix.filter((r) => r.current).length,
    blocked: matrix.filter((r) => !r.current).length,
  }), [matrix]);

  const exportCsv = () => {
    const esc = (v: string) => `"${(v || "").replace(/"/g, '""')}"`;
    const head = ["Member", "Email", "Role", "Completed", "Member-contact current", ...modules.map((m) => m.code)].map(esc).join(",");
    const lines = matrix.map((row) => [
      row.staff.display_name || row.staff.name || row.staff.email || "",
      row.staff.email || "",
      row.staff.role,
      `${row.completed}/${modules.length}`,
      row.current ? "yes" : "no",
      ...row.rows.map((r) => (r.status === "passed" || r.status === "acknowledged" ? (r.score != null ? `${Math.round(r.score * 100)}%` : "ack") : r.status)),
    ].map(esc).join(","));
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="grid sm:grid-cols-3 gap-3 mb-6">
        <div className="rounded-lg border border-border bg-card p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-body tracking-[0.2em] uppercase text-muted-foreground">Staff</p>
            <p className="font-display text-2xl text-foreground">{summary.total}</p>
          </div>
        </div>
        <div className="rounded-lg border border-border bg-card p-4 flex items-center gap-3">
          <ShieldCheck size={16} className="text-primary" />
          <div>
            <p className="text-[10px] font-body tracking-[0.2em] uppercase text-muted-foreground">Current</p>
            <p className="font-display text-2xl text-foreground">{summary.current}</p>
          </div>
        </div>
        <div className="rounded-lg border border-border bg-card p-4 flex items-center gap-3">
          <ShieldAlert size={16} className="text-red-400" />
          <div>
            <p className="text-[10px] font-body tracking-[0.2em] uppercase text-muted-foreground">Stand down</p>
            <p className="font-display text-2xl text-red-400">{summary.blocked}</p>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {matrix.map((row) => (
              <tr key={row.staff.user_id} className="border-t border-border hover:bg-foreground/[0.03] transition-colors">
                ...
                <td className="px-3 py-3 text-center text-foreground font-body">{row.completed}/{modules.length}</td>
                {row.rows.map((r) => {
                  const completed = r.status === "passed" || r.status === "acknowledged";
                  return (
            ...
            {matrix.length === 0 && (
              <tr><td colSpan={4 + modules.length} className="px-4 py-12 text-center text-muted-foreground">No staff found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
