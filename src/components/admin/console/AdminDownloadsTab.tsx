...
const AdminDownloadsTab = () => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast.success("Colouring page ready", { description: "A4 PDF generated â€” download it below." });
    } catch (e) {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h2 className="font-display text-2xl text-primary tracking-wider">Downloads</h2>
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="text-[11px] font-body text-muted-foreground mb-4">
          One click generates the week's colouring page as a branded A4 PDF â€” MINDCAST logo, session date, title and a "Your Name" line. No Canva, no manual layout.
        </p>
        <div className="flex items-center gap-3">
          ...
          <button onClick={generate} disabled={busy}
            ...
            {busy ? "Generatingâ€¦" : "Generate A4 PDF"}
          </button>
          {result && (
            <>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Link to="/portal/downloads"
          ...
          <span className="text-[11px] font-body text-muted-foreground">Member download library â€” admins see every week unlocked.</span>
        </Link>
      </div>
