-- Database-side reporting queries for the Admin-only Membership Health page.
-- Requires 20260901120000_admin_reporting_foundation.sql.

-- Track resolution lives in one place. age_group is authoritative for youth;
-- membership_tier supplies the fallback for legacy profiles.
CREATE OR REPLACE FUNCTION public.reporting_profile_track(
  p_age_group text,
...
AS $$
  SELECT CASE
    WHEN lower(COALESCE(p_age_group, '')) = 'child'
      OR lower(COALESCE(p_membership_tier, '')) = 'child' THEN 'Child'
    WHEN lower(COALESCE(p_age_group, '')) = 'teen'
      OR lower(COALESCE(p_membership_tier, '')) = 'teen' THEN 'Teen'
    ELSE 'Adult'
  END;
$$;

CREATE OR REPLACE VIEW public.reporting_member_dimension AS
SELECT
  p.id AS profile_id,
  COALESCE(NULLIF(p.display_name, ''), NULLIF(p.name, ''), p.email, 'Member') AS member_name,
  p.email,
  public.reporting_profile_track(p.age_group, p.membership_tier) AS track,
  p.membership_status,
  p.membership_tier,
  p.created_at AS registered_at,
  p.cohort AS programme_cohort,
  p.home_location_id AS location_id,
  l.name AS location_name,
  hm.household_id,
  h.name AS household_name,
  first_active.started_at AS observed_membership_started_at,
  first_active.is_baseline AS observed_start_is_baseline
FROM public.profiles p
LEFT JOIN public.programme_locations l ON l.id = p.home_location_id
LEFT JOIN LATERAL (
...
LEFT JOIN LATERAL (
  SELECT e.effective_at AS started_at, e.is_baseline
...
REVOKE ALL ON public.reporting_member_dimension FROM PUBLIC, anon, authenticated;

-- One row per member and eligible scheduled session. `check_ins` remains the
-- source of truth; EXISTS makes duplicate NFC/kiosk/manual scans count once.
CREATE OR REPLACE VIEW public.reporting_eligible_session_outcomes AS
SELECT DISTINCT
  d.profile_id,
  d.member_name,
  d.track,
  ...
  d.household_id,
  d.household_name,
  d.location_id AS member_location_id,
  s.id AS scheduled_session_id,
  s.session_date,
  s.starts_at,
  s.week_number,
  s.location_id,
  l.name AS location_name,
  s.facilitator_profile_id,
  EXISTS (
    SELECT 1
    FROM public.check_ins ci
    WHERE ci.profile_id = d.profile_id
      AND (
        ci.scheduled_session_id = s.id
        OR (
          ci.scheduled_session_id IS NULL
          AND COALESCE(ci.track, 'Adult') = s.track
          AND (ci.checked_in_at AT TIME ZONE COALESCE(l.timezone, 'Pacific/Auckland'))::date = s.session_date
        )
      )
  ) AS attended
...
JOIN public.scheduled_sessions s
  ON s.track = d.track
 AND s.status <> 'cancelled'
 AND s.session_date <= (now() AT TIME ZONE 'Pacific/Auckland')::date
LEFT JOIN public.programme_locations l ON l.id = s.location_id
WHERE (
    (s.session_date::timestamp + time '12:00')
      AT TIME ZONE COALESCE(l.timezone, 'Pacific/Auckland')
  ) >= mi.starts_at
  AND (
    mi.ends_at IS NULL
    OR (
  ...
  AND (d.location_id IS NULL OR s.location_id IS NULL OR d.location_id = s.location_id)
  AND s.session_date >= COALESCE(
    (SELECT value::timestamptz::date FROM public.app_settings WHERE key = 'reporting_history_started_at'),
    current_date
  );

REVOKE ALL ON public.reporting_eligible_session_outcomes FROM PUBLIC, anon, authenticated;

-- Adult-only completion state. Available fields are the five always-present
-- journal inputs plus each configured video question. Follow-up intention
-- outcome/reflection fields are not part of same-session completion.
CREATE OR REPLACE VIEW public.reporting_journal_session_completion AS
SELECT
  o.profile_id,
  o.scheduled_session_id,
  o.session_date,
  o.week_number,
  o.location_id,
  o.location_name,
  o.attended,
  COALESCE(ls.session_title, ls.theme_title, 'Week ' || o.week_number::text) AS theme,
  5
    + CASE WHEN NULLIF(btrim(ls.video_question_1), '') IS NOT NULL THEN 1 ELSE 0 END
    + CASE WHEN NULLIF(btrim(ls.video_question_2), '') IS NOT NULL THEN 1 ELSE 0 END
    AS fields_available,
  COALESCE(completed.fields_completed, 0)::integer AS fields_completed,
  completed.last_activity_at,
  opened.first_opened_at AS journal_opened_at
FROM public.reporting_eligible_session_outcomes o
LEFT JOIN LATERAL (
  SELECT m.session_title, m.theme_title, m.video_question_1, m.video_question_2
  FROM public.mindcast_live_sessions m
  WHERE m.week_number = o.week_number AND m.audience = 'Adult'
  ORDER BY m.updated_at DESC
  LIMIT 1
) ls ON true
LEFT JOIN LATERAL (
  SELECT count(*) FILTER (
           WHERE js.event = 'completed'
             AND (
               js.field_key NOT IN ('video_question_1_response', 'video_question_2_response')
               OR (js.field_key = 'video_question_1_response' AND NULLIF(btrim(ls.video_question_1), '') IS NOT NULL)
               OR (js.field_key = 'video_question_2_response' AND NULLIF(btrim(ls.video_question_2), '') IS NOT NULL)
             )
         )::integer AS fields_completed,
         max(js.occurred_at) FILTER (WHERE js.event = 'completed') AS last_activity_at
  FROM public.reporting_journal_field_state js
  WHERE js.profile_id = o.profile_id
    AND js.week_number = o.week_number
    AND js.track = 'Adult'
) completed ON true
LEFT JOIN public.journal_session_activity opened
  ON opened.profile_id=o.profile_id AND opened.week_number=o.week_number AND opened.track='Adult'
WHERE o.track = 'Adult';

REVOKE ALL ON public.reporting_journal_session_completion FROM PUBLIC, anon, authenticated;

-- Main filtered payload. All aggregation happens in Postgres so the browser
-- does not download private journals or complete histories.
    ...
    'relationships', jsonb_build_object(
      'retention_by_attendance_band', (
        SELECT COALESCE(jsonb_agg(jsonb_build_object(
          'band',b.label,'members',count(rm.profile_id),
          'retained',count(rm.profile_id) FILTER (WHERE rm.retained_eight_weeks),
          'retention',count(rm.profile_id) FILTER (WHERE rm.retained_eight_weeks)::numeric/NULLIF(count(rm.profile_id),0)*100
...
CREATE OR REPLACE FUNCTION public.admin_reporting_member_detail(p_profile_id uuid)
RETURNS jsonb
LANGUAGE plpgsql STABLE SECURITY DEFINER
SET search_path = public
AS $$
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'membership', jsonb_build_object(
      'profile_id',d.profile_id,'name',d.member_name,'email',d.email,'track',d.track,
      'household',d.household_name,'status',d.membership_status,'tier',d.membership_tier,
      'registered_at',d.registered_at,'observed_membership_started_at',
        CASE WHEN d.observed_start_is_baseline THEN NULL ELSE d.observed_membership_started_at END,
      'start_is_known',NOT COALESCE(d.observed_start_is_baseline,true),
      'membership_duration_days',CASE WHEN NOT COALESCE(d.observed_start_is_baseline,true)
        THEN floor(extract(epoch FROM (now()-d.observed_membership_started_at))/86400) END,
      'location',d.location_name
    ),
    'attendance', (
      SELECT jsonb_build_object(
        'eligible',count(*),'attended',count(*) FILTER (WHERE attended),'missed',count(*) FILTER (WHERE NOT attended),
        'rate',count(*) FILTER (WHERE attended)::numeric/NULLIF(count(*),0)*100,
        'last_attendance',max(session_date) FILTER (WHERE attended),
        'history',COALESCE(jsonb_agg(jsonb_build_object(
          'session_date',session_date,'week',week_number,'attended',attended,'location',location_name
        ) ORDER BY session_date DESC),'[]'::jsonb)
      ) FROM public.reporting_eligible_session_outcomes o WHERE o.profile_id=d.profile_id
    ),
    'journal', CASE WHEN d.track <> 'Adult' THEN jsonb_build_object(
      'supported',false,'reason','Teen and Child tracks are paper-only; digital journal metrics do not apply.'
    ) ELSE (
      SELECT jsonb_build_object(
        'supported',true,'sessions_with_activity',count(*) FILTER (WHERE fields_completed>0),
        'fields_available',sum(fields_available),'fields_completed',sum(fields_completed),
        'completion_rate',sum(fields_completed)::numeric/NULLIF(sum(fields_available),0)*100,
        'most_recent_activity',max(last_activity_at),
        'sessions',COALESCE(jsonb_agg(jsonb_build_object(
          'week',week_number,'date',session_date,'available',fields_available,
          'completed',fields_completed,'completion',fields_completed::numeric/NULLIF(fields_available,0)*100
        ) ORDER BY session_date DESC),'[]'::jsonb)
      ) FROM public.reporting_journal_session_completion j WHERE j.profile_id=d.profile_id
    ) END
  ) INTO v_result
  FROM public.reporting_member_dimension d WHERE d.profile_id=p_profile_id;
  RETURN v_result;
END;
$$;

REVOKE ALL ON FUNCTION public.admin_reporting_member_detail(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_reporting_member_detail(uuid) TO authenticated;

-- CSV source. It reuses the exact dashboard payload so filters and definitions
-- cannot drift between charts and downloads.
CREATE OR REPLACE FUNCTION public.admin_reporting_export(
  p_report text,
  p_start_date date DEFAULT NULL,
  p_end_date date DEFAULT NULL,
  p_track text DEFAULT NULL,
...
SET search_path = public
AS $$
DECLARE v_data jsonb; v_path text;
BEGIN
  IF NOT public.has_role(auth.uid(), 'admin'::app_role) THEN RAISE EXCEPTION 'not authorized'; END IF;
  v_data := public.admin_reporting_dashboard(p_start_date,p_end_date,p_track,p_status,p_location_id,p_cohort);
  v_path := CASE p_report
    WHEN 'member_metrics' THEN 'member_metrics'
    WHEN 'attendance' THEN 'member_metrics'
    WHEN 'retention_cohorts' THEN 'cohorts'
    WHEN 'session_engagement' THEN 'sessions'
    WHEN 'journal_engagement' THEN NULL
    ELSE NULL END;

  IF p_report = 'journal_engagement' THEN
    RETURN QUERY SELECT value FROM jsonb_array_elements(COALESCE(v_data #> '{journal,trend}','[]'::jsonb));
  ELSIF v_path IS NOT NULL THEN
    RETURN QUERY SELECT value FROM jsonb_array_elements(COALESCE(v_data -> v_path,'[]'::jsonb));
  ELSE
...
REVOKE ALL ON FUNCTION public.admin_reporting_export(text,date,date,text,text,uuid,text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_reporting_export(text,date,date,text,text,uuid,text) TO authenticated;
